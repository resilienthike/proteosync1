...
def _pre_minimize(top, pos):
    ...
    integ  = _Integrator(300*unit.kelvin, 1.0/unit.picosecond, 0.002*unit.picoseconds)
...
def _align_long_axis_to_z(top, pos):
    ...
    v = np.cross(u, z); s = np.linalg.norm(v); c = float(np.dot(u, z))
    if s < 1e-8:
...
def prepare_system(seed_file: Path, out_dir: Path, lipid: str="POPC", salt_molar: float=0.15, padding_nm: float=2.0, reorient: bool=True):
    ...
    integ = _Integrator(310*unit.kelvin, 1.0/unit.picosecond, 0.004*unit.picoseconds)
    ...
    with open(state_xml, "w") as fh:
        fh.write(XmlSerializer.serialize(sim.context.getState(getPositions=True, getVelocities=True)))
...
def main():
    ...
    if not seed.exists():
        raise FileNotFoundError("seed_structure.(pdb|cif) not found under artifacts/data/<target>/")
