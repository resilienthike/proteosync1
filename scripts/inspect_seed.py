...
def inspect_structure(target_name: str):
    ...
    for model in structure:
        for chain in model:
            chain_lengths[chain.id] = sum(1 for residue in chain if is_aa(residue, standard=True))

    print("\n--- Summary ---")
    print(f"Chain lengths (amino acids): {chain_lengths}")

    # 4. Identify receptor and peptide candidates
    receptor_chain = max(chain_lengths, key=chain_lengths.get) if chain_lengths else None
    peptide_candidates = [cid for cid, length in chain_lengths.items() if 5 <= length <= 120]
    
    print(f"Identified Receptor Chain: {receptor_chain} (length {chain_lengths.get(receptor_chain)})")
